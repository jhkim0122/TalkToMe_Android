...
public class MainActivity extends AppCompatActivity implements Scene.OnUpdateListener {
    ...
    private final GestureDetector.OnDoubleTapListener mDoubleTapListener = new GestureDetector.OnDoubleTapListener() {
        ...
        public boolean onDoubleTapEvent(MotionEvent motionEvent) {
            ...
            if (image != null) {
                ...
                tts1.speak(results.toString(), TextToSpeech.QUEUE_FLUSH, params, results.toString());
            }
            return false;
        }
    };
    ...
    public void onUpdate(FrameTime frameTime) {
        ...
        if (distance > 0.3 && distance <= 0.5) {
            ...
            tts2.speak(text, TextToSpeech.QUEUE_FLUSH, params, text);
        }
        if (distance > 0.2 && distance <= 0.3) {
            vibrator.vibrate(VibrationEffect.createWaveform(vibratePattern2, 3));
            tts2.speak(text, TextToSpeech.QUEUE_FLUSH, params, text);
        }
        if (distance < 0.2) {
        ...
        if (distance < 0.5) {
            ...
            tts2.speak("", TextToSpeech.QUEUE_FLUSH, params, "");
        }
